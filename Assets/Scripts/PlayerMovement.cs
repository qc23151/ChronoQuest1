...
//     {
//         // Only run this check if we are currently in the Base (Ground/Idle) state
//         if (animAuthority != AnimAuthority.Base) return;

//         // FIX: If we lose the ground but didn't press jump (e.g., walked off a ledge)
//         if (!isGrounded)
//         {
//             // 1. Switch authority to Airborne immediately
//             animAuthority = AnimAuthority.Airborne;
            
//             // 2. Trigger the "Jump" animation state so the Animator leaves the Idle/Run blend tree
//             // (This ensures we enter the state where 'SetFrame' actually works)
//             anim.SetTrigger("Jump"); 
//             anim.SetBool("isGrounded", false);
//             return;
//         }

//         // Standard ground behavior
//         anim.SetFloat("Speed", Mathf.Abs(horizontalInput));
//         anim.SetBool("isGrounded", isGrounded);
//     }
