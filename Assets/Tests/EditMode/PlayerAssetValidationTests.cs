...
{
    /// <summary>
    /// Tests that validate player-related assets and components exist and are properly configured.
    /// </summary>
    [TestFixture]
    public class PlayerAssetValidationTests
    {
        [Test]
        public void ValidSprite()
        {
            // WARNING: This filepath will need to change when we update the sprite used for main player 
            GameObject playerSprite = AssetDatabase.LoadAssetAtPath<GameObject>("Assets/Sprites/MeleeSkeleton/Skull Boy.aseprite");
        ...
        {
            MonoScript script = AssetDatabase.LoadAssetAtPath<MonoScript>("Assets/Scripts/PlayerMovement.cs");
